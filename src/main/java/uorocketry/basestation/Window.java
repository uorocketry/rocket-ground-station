...
public class Window extends JFrame {
	...
	public Window(Main main, Config config) {
		...
		for (int i = 0; i < config.getDatasets().length; i++) {
			generateTelemetryPanel(i, config.getDatasets()[i]);
		}
		...
		chartDataPointsOptions.setBorder(
				new TitledBorder(null, "Chart Data Points", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		for (int i = 0; i < config.getDatasets().length; i++) {
			addSlider(config.getDatasets()[i]);
		}
		...
		try {
			StateEvent[] stateEvents = config.getStateEvents();
			...
			List<StateButton> buttons = new ArrayList<StateButton>(stateEvents.length);
			stateButtons.add(buttons);

			for (int i = 0; i < stateEvents.length; i++) {
				StateEvent stateEvent = stateEvents[i];
				StateButton stateButton = new StateButton(main.deviceConnectionHolder,
						config.getDatasets()[StateButton.TABLE_INDEX], stateEvent);
			...
			if (buttons.size() > 0) {
				addComSelectorPanel(DeviceConnectionHolder.Type.BUTTON_BOX, "Button Box",
						buttons.stream().toArray(StateButton[]::new));
			}
		} catch (Exception e) {
			// No states then
		...
		for (int i = 0; i < config.getDatasets().length; i++) {
			addComSelectorPanel(config.getDatasets()[i], main);
		}
	...
	public void generateTelemetryPanel(int tableIndex, DataSet dataSet) {
		...
		JTable receivedDataTable = createTable(config.getDatasets()[tableIndex].getLabels().length, 2, 30, 130);
	...
	public void addSlider(DataSet dataSet) {
		...
		sliderTabs.add(sliders, dataSet.getName());
	}

	public void addComSelectorPanel(DataSet dataSet, DataReceiver... dataReceivers) {
		addComSelectorPanel(DeviceConnectionHolder.Type.TABLE, dataSet.getName(), dataReceivers);
	}
